...
def Config(file:str = "config.json", template: dict | None= None):
    ...
    try:
        ...
        create_config(file, template)
        console.info("Config file created, proceeding")
        return Config(file, template)
...
def create_config(file: str, template: dict):
    if file == None: raise notification.error("Cannot create config file 'None'")
    try:
        open(file, "x").write(json.dumps(template, indent=4))
