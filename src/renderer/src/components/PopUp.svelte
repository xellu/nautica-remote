...
{#if open && loaded}
    <div class="w-screen h-screen fixed top-0 left-0 flex items-center justify-center bg-black/50 z-50"
        onkeydown={() => {}}
        role="checkbox"
        tabindex=0
        ...
        <!-- form -->
        <div class="card bg-surface-900 w-full z-50 {maxW ? maxW : ''}" transition:scale={{duration: 250}}>
            ...
            <div class="flex justify-between items-center select-none w-full card variant-filled-surface p-3 gap-10 {headerCss}">
                ...
                <button onclick={() => {if (!preventClose) { open = false }}} title="close">
                    ❌
                </button>
            </div>
